...
- 客户端负责连接主机、上传 JPG/JPEG/PNG、查看图片墙、打星、分类、修改状态、下载原图/待修包、上传已修图。
...
当前导入与客户端上传支持 JPG/JPEG/PNG。暂不支持 RAW、NEF、ARW、CR3、HEIC、TIFF、GIF、WebP 原图、视频。代码可预留扩展点，但不要实现 RAW。

## 5. 缩略图与预览图

每张导入图片生成：

- `original`：原始 JPG/JPEG/PNG，用于修图、导出；轻量归档只记录其历史路径，不再默认复制原图。
...
- 图片导入页：主机本地导入、客户端上传导入、远程传输预留；主机端支持选择文件夹导入或手动多选图片文件导入，第一版只实现主机本地与客户端上传两类入口。
- 图片墙页：缩略图网格、左侧筛选、顶部工具栏、右侧信息栏、批量操作、快捷键、打星、状态、分类、标签、下载、导出当前筛选。
- 图片墙响应式要求：宽屏显示筛选栏、图片墙和元数据栏；中窄窗口下筛选栏和元数据栏应可折叠或抽屉化，顶部次要操作进入“更多操作”，按钮文本不得被挤成竖排。
...
开发模式下，前端 Vite 服务应监听 `0.0.0.0:5173`，同局域网设备访问前端页面使用 `http://主机局域网IP:5173`；后端 API 仍使用 `http://主机局域网IP:{实际后端端口}`，不要把后端端口写死到前端业务逻辑。

生产 / 打包模式下不再依赖 Vite `5173`。Express 必须托管 `dist/`，客户端浏览器访问 `http://主机局域网IP:{实际后端端口}` 即可打开前端页面；API 健康检查为 `http://主机局域网IP:{实际后端端口}/api/health`。非 `/api`、非 `/socket.io` 的前端路由请求应回退到 `index.html`，避免 React Router 页面刷新 404。

Windows 打包目标为便携 ZIP 包和 NSIS 安装包，输出到 `release-pack/`。便携版以“解压 ZIP 后运行 `Media Photo Workbench.exe`”为当前交付形式，不把单文件 self-extract portable EXE 作为主交付物。运行时数据 `data/app.db`、`config/config.json` 和 `logs/` 在生产模式下应写入 Electron `app.getPath("userData")` 等可写目录，不要写入只读安装目录。`release/`、`release-pack/`、`release-win/`、`dist/`、`dist-server/`、`data/`、`logs/`、真实仓库、`working/`、`archive/` 和 ZIP 产物不得提交 Git。
...
客户端上传流程：连接主机、选择活动、摄影师、设备、JPG/JPEG/PNG、上传主机、主机入库、生成缩略图/预览图、写 SQLite、广播新图片。
...
主机端可全部生成一个待修包、按包数量平均拆分待修包，也可按图片内容或人员分工自定义分包，用于多人同时修图。每个待修包都必须包含独立的 `edit_manifest.json`、`待修原图/` 和 `已修图回传/`，manifest 需记录包名和包序号；主机端可删除误生成的待修包 ZIP 和待修包记录，但不得影响待修图片、原图或已修图；客户端可以查看、下载待修包并在客户端页面回传已修图。客户端第一版不允许生成或删除待修包。
