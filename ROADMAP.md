...
项目已经进入 Phase 3：主机本地图片导入与处理管线。当前代码已完成扫描和同步导入管线，等待安装 `sharp` / `exifr` 后进行完整图片处理端到端验证。
...
### Phase 3 验证与收尾
- 安装 `sharp` / `exifr` 并更新 `pnpm-lock.yaml`。
- 使用真实 JPG/JPEG 验证完整导入、缩略图、预览图、EXIF 和去重。
- 根据验证结果决定是否把同步导入改造成任务队列。
