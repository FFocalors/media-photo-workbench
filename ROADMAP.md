...
项目已进入 v0.11.5-dev：修图协作与导航流程优化。当前代码已经支持主机本地导入、真实图片墙、基础选片、单图下载、Socket.IO 实时同步、客户端上传协作、修图流转、发布导出、活动归档、活动/图片回收站恢复和安全永久删除、统一任务中心、批量 ZIP 下载、working 清理后的历史归档只读查看、客户端下载待修包和回传已修图，并支持单包、平均拆包和按人员分工自定义待修分包。开发模式下 Vite 前端服务已监听 `0.0.0.0:5173`，便于后续真实局域网和移动端浏览器测试。
