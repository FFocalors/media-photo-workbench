...
当前进入 v0.14.0-rc：发布候选测试与稳定性优化。项目已完成 v0.13.2-dev Windows ZIP 便携预发布版，代码已经支持主机本地 JPG/JPEG/PNG 导入、真实图片墙、基础选片、单图下载、Socket.IO 实时同步、客户端上传协作、修图流转、发布导出、活动归档、活动/图片回收站恢复和安全永久删除、统一任务中心、批量 ZIP 下载、working 清理后的历史归档只读查看、客户端下载待修包和回传已修图，并支持单包、平均拆包和按人员分工自定义待修分包。v0.14 已将主机导入、客户端上传后的图片处理和归档工作区清理纳入后台任务，任务中心显示进度、预计剩余时间和任务结果，避免大批量导入或多图活动清理 `working` 时页面长时间卡住；同时修复关闭应用时导入任务继续写日志导致主进程报错的风险。v0.12 已完成桌面端非最大化窗口适配；v0.13 已完成生产模式前端托管、Electron Builder 配置、原生依赖处理、真实运行端口透传、主机首页剩余空间真实显示、二维码生成、ZIP 便携包验证、多设备测试和 GitHub Release 发布。推荐交付物为 `MediaPhotoWorkbench-v0.13.2-dev-x64.zip`；单文件 self-extract portable EXE 不作为当前主交付物。NSIS 安装包在 v0.14.0-rc 人工测试中出现安装器卡住，已移入后续待评估项，不作为当前发布候选阻塞项。
