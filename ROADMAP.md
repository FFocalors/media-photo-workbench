...
当前进入 v0.16.0 开发阶段。v0.15.0 已发布，本阶段先以 16.1、16.2、16.3、16.4、16.5 等内部批次处理实战问题，最终统一进入 `v0.16.0` tag，不单独发布内部批次版本。16.1 第一批修复聚焦图片墙实时预览、批量状态修改、筛选预览体验和星级精确筛选：实时接收新图后图片资源增加有限重试和缓存键，批量状态修改基于选中 ID 快照执行并汇总成功 / 失败数量，预览弹窗在筛选状态下冻结当前浏览列表，星级筛选新增“等于 / 及以上”两种模式。16.2 聚焦任务中心一致性和业务页面结果同步：导入页和客户端上传页会监听对应 `task-updated`，与任务中心共用同一套任务统计映射；待修包、发布导出和归档 prepare 的任务中心记录补充业务级成功 / 失败 / 跳过统计。16.3 聚焦任务中心覆盖补齐：已修图回传接入 `edited_upload` 后台任务，主机和客户端回传页面同步显示任务进度；任务中心统一上传处理、批量 ZIP、发布导出、待修包、已修图回传和归档清理等耗时操作的名称、统计、错误和下载入口。16.4 聚焦拖拽导入 / 拖拽上传体验增强：主机导入页支持拖拽 JPG/JPEG/PNG 图片文件或文件夹，文件夹仍只扫描第一层；客户端上传页支持拖拽单张或多张 JPG/JPEG/PNG 图片，客户端拖拽文件夹暂不支持并显示明确提示；拖拽入口复用现有导入、上传、去重和任务中心流程。16.5 聚焦数据库备份与数据库位置迁移：设置页支持立即备份数据库到当前仓库 `metadata/database-backups/`，启动时按 24 小时节流自动备份，自动备份保留最近 10 份；数据库路径独立于图片仓库路径，可迁移到新目录，迁移前强制备份，失败时回滚配置，旧数据库和用户图片仓库均不会删除。项目已完成 v0.15.0-rc.0 发布候选版，代码已经支持主机本地 JPG/JPEG/PNG 导入、真实图片墙、基础选片、单图下载、Socket.IO 实时同步、客户端上传协作、修图流转、发布导出、活动归档、活动/图片回收站恢复和安全永久删除、统一任务中心、批量 ZIP 下载、working 清理后的历史归档只读查看、客户端下载待修包和回传已修图，并支持单包、平均拆包和按人员分工自定义待修分包。v0.15.0-rc.0 将主机导入、客户端上传后的图片处理和归档工作区清理纳入后台任务，任务中心显示进度、预计剩余时间和任务结果，避免大批量导入或多图活动清理 `working` 时页面长时间卡住；同时修复关闭应用时导入任务继续写日志导致主进程报错的风险。v0.12 已完成桌面端非最大化窗口适配；v0.13 已完成生产模式前端托管、Electron Builder 配置、原生依赖处理、真实运行端口透传、主机首页剩余空间真实显示、二维码生成、ZIP 便携包验证、多设备测试和 GitHub Release 发布。v0.15.0-rc.0 推荐交付物为 `MediaPhotoWorkbench-v0.15.0-rc.0-x64.zip`；单文件 self-extract portable EXE 不作为当前主交付物。NSIS 安装包在 v0.14.0 人工测试中出现安装器卡住，已移入后续待评估项，不作为当前发布候选阻塞项。
...
- **v0.14.0**：故障排查 / 诊断信息入口、客户端连接失败提示、校园网 / 防火墙 / 热点排查提示、NSIS 初步验证；ZIP 便携包继续作为主交付物。
- **v0.15.0-rc.0**：发布候选版，导入性能与稳定性增强；包含大批量导入任务化、PNG 支持、主机文件夹 / 指定图片导入、客户端 JPG/JPEG/PNG 上传、图片墙分页修复、归档清理工作区任务化和关闭时日志崩溃风险修复。
- **v0.15.0 或 v0.15.0-rc.1**：根据回归测试结果决定是否正式发布或继续修复。
- **v1.0.0**：第一个稳定可用版本。
