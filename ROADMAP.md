...
当前进入 v0.17.0 开发阶段。v0.16.0 已发布，本阶段目标是现场协作与选片效率增强，内部阶段编号使用 17.1、17.2 等临时批次，但不作为正式版本号、Git tag 或 Release 名称；最终统一进入 `v0.17.0`。17.1 聚焦现场协作感知与来源追踪：客户端连接后通过 Socket.IO 上报持久化 `clientId` 和设备名称，主机首页与图片墙显示在线客户端数量，主机首页显示在线客户端列表；客户端上传图片写入上传者、上传角色和上传时间；图片元数据展示来源、上传者和上传时间；图片墙支持按主机导入或具体客户端设备筛选；星级、状态、分类、备注、删除和恢复操作日志记录操作者；主机首页以内存方式显示最近上传、最近修改、删除 / 恢复和任务完成动态。17.2 聚焦图片墙效率增强：快捷键提示面板、批量分类和批量操作后清空 / 保留选择配置；根据现场使用反馈，移除筛选预设和快捷筛选入口。17.1 / 17.2 已测试通过。17.1 / 17.2 不引入账号系统、登录系统、权限系统、云同步、远程传输、RAW/HEIC/视频或打包发布策略调整。
