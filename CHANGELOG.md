...
- v0.14.0-rc 第四项继续优化大批量导入：主机导入和客户端上传处理改为后台任务，`import/start` 与 `upload` 返回 `taskId`，任务中心通过 `task-updated` 实时显示导入进度。
...
- v0.14.0-rc 第四项新增 PNG 图片支持：主机本地导入、客户端上传、WebP 缩略图/预览图、图片墙展示、原图下载、批量 ZIP 和待修包均兼容 PNG 原图。
