...
- 合并自定义透明窗口外壳、顶部活动摘要、原生窗口控制与 Windows 11 圆角布局。
- 左侧栏增加“已连接客户端”入口，统一客户端身份、在线状态和 Socket.IO 实时刷新。
- 增加移动端轻量连接页和图片墙，支持触控筛选、预览、打星、状态、备注与受控下载，并限制移动端访问桌面重操作页面。
- 完善 IIS/FTP 未初始化、服务依赖、ACL、Authorization Deny、活动切换回滚、提权长任务恢复与真实阶段进度。
- ZIP 便携版根目录收敛为启动入口与 `runtime/`；`v2.1.0` 产物需从 Tag 重新构建并通过异机验证后再发布。

### v1.2.2 - 提权长任务与真实进度修复

- 修复异机 `%TEMP%` 无法建立严格 ACL 时初始化直接失败：改用当前用户 SID 分步授权，并在系统 TEMP 不可安全使用时回退本地应用数据目录。
- `TEMP_ACL_FAILED` 现在包含 `secure_temp_directory` 阶段、脱敏失败步骤和父子 operationId；该阶段在 UAC 与系统修改前，固定报告“未修改系统，无需回滚”。
- setup、repair、start、restart 和 adopt 的 provisioning 上限调整为 20 分钟；普通检测、停止、账户等操作继续使用短超时。
- 提权执行器向管理员脚本传递 `StatusPath / OperationId`，脚本在 Windows 功能、IIS 初始化、账户、ACL、站点、PASV、防火墙、FTPSVC、监听与最终验证前原子写入脱敏真实阶段。
- 新增只读 `GET /api/camera-ftp/admin-operation`；页面每秒显示真实节点进度、已等待时间、阶段区间估算、估算超期提示和不可读取内部百分比时的不确定动画。
- 达到 20 分钟后不强制结束可能仍在修改 Windows 的管理员进程，不声称已回滚，不转换成重启建议，并在进程结束前禁止重试。
- 后端启动时恢复遗留提权操作：进程存在时继续监控，进程消失时标记 abandoned 并允许重新检测，迟到结果只用于诊断而不复用密码或确认。
- 状态文件只保留操作标识、脚本、阶段、PID 和时间，不包含密码、完整路径、风险确认或提权输入；写入使用临时文件原子替换。
- 本轮只生成 Windows ZIP 便携包供异机测试，不创建 Tag、GitHub Release 或 NSIS 安装包。

### v1.2.1 - 内部异机测试包

- 将 `v1.2.0-alpha.1` 的 IIS/FTP 健壮性与使用体验改进合并到 `main`，版本统一为 `1.2.1`。
- 完善 IIS/FTP 未启用、未初始化、未启动、组件 Pending、服务禁用/停止/等待、依赖异常和配置未就绪等状态的安全自动修复与细分诊断。
- 修复无关 Windows 待重启标记误阻断、FTPSVC 已运行仍误报启动失败、普通权限端口检测 Access Denied 导致假停止，以及最终监听验证耗时过长的问题。
- 停止 FTP 只停止工作台托管站点；页面在部分权限状态下仍可发起安全停止，并以白底、红色边框和文字提示停止操作。
