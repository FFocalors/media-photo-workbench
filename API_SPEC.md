...
前端停止操作以统一的 `serviceReady` 运行态为准，不要求普通权限必须读取到 `site.started=true`。当 `site.started=null` 但托管配置已初始化、FTPSVC Running 且目标端口监听明确时，可以调用 `POST /api/camera-ftp/stop`；服务端仍通过管理员脚本和 managed Site ID 校验目标，只停止工作台站点，不停止共享 FTPSVC。
