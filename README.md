...
项目已打通从主机建活动、本地导入、真实图片墙、单图下载、Socket.IO 实时同步、客户端上传协作、待修包生成、已修图回传、正式发布导出 ZIP、活动归档，到活动/图片回收站恢复和安全永久删除的核心闭环。当前已接入统一任务中心、批量 ZIP 下载、导入后台任务、归档活动只读打开、客户端修图任务页、自定义待修分包和桌面端非最大化窗口适配。v0.13.2-dev 已发布到 GitHub Release，本机测试、多设备测试、压力测试和主机 Windows 热点连接测试均已通过；当前 v0.14.0-rc 重点进行连接诊断、日志排查、真实活动压力复核和发布前稳定性优化。
...
- 主机本地 JPG/JPEG/PNG 导入，支持选择文件夹批量导入，也支持从文件夹中手动选择单张或多张图片导入；大批量导入通过后台任务执行，任务中心显示进度、预计剩余时间和取消入口。
...
- 任务中心与批量 ZIP 下载：提供内存任务队列、`task-updated` 实时推送、主机/客户端侧边栏任务中心、图片墙“下载所选 ZIP”；主机导入、客户端上传处理、发布导出和归档生成均可在任务中心查看进度和错误。
...
真实压力测试建议使用 50 / 300 / 500 张 JPG/JPEG/PNG 分批验证导入、图片墙滚动、批量下载、待修包、导出发布和归档；大批量导入建议额外用 4000 张混合素材验证后台任务进度、ETA、取消、重复导入 skipped 和关闭程序安全性。第一版暂不把手机端作为完整工作流适配目标。
