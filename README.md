...
**当前版本：0.10.0-dev（任务队列与批量 ZIP 下载阶段）**

项目已打通从主机建活动、本地导入、真实图片墙、单图下载、Socket.IO 实时同步、客户端上传协作、待修包生成、已修图回传、正式发布导出 ZIP、活动归档，到活动/图片回收站恢复和安全永久删除的核心闭环。当前已开始接入统一任务中心和批量 ZIP 下载，仍处于开发阶段，尚未实现归档活动只读打开和远程传输预留。
...
- 活动归档：生成 `archive/{event_slug}` 归档目录、复制原图/已修图/导出文件，生成 `manifest.json`、`images.csv`、`operation_logs.csv` 和独立 `event.db`，支持归档验证和清理工作区。
- 回收站与永久删除：支持图片回收站、活动回收站、恢复、二次确认永久删除；活动永久删除默认不删除归档目录。
...
下阶段重点：
- 归档活动只读打开。
- 将导入、待修包、已修图回传、清理工作区等更多耗时操作接入任务系统。
- 远程传输预留入口。
