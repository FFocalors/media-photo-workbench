...
> **注意**：当前导入与客户端上传支持 JPG/JPEG/PNG 格式，暂不支持 RAW、HEIC 或视频格式。已修图回传仍建议使用 JPG/JPEG 成片。图片物理文件绝不存入 SQLite，数据库仅记录文件路径和生命周期状态。
...
**当前版本：v0.15.0-rc.0（Windows ZIP 便携发布候选版）**

项目已打通从主机建活动、本地导入、真实图片墙、单图下载、Socket.IO 实时同步、客户端上传协作、待修包生成、已修图回传、正式发布导出 ZIP、活动归档，到活动/图片回收站恢复和安全永久删除的核心闭环。当前已接入统一任务中心、批量 ZIP 下载、导入后台任务、归档活动只读打开、客户端修图任务页、自定义待修分包和桌面端非最大化窗口适配。v0.15.0-rc.0 将大批量导入任务化、PNG 支持、主机文件夹 / 指定图片导入、客户端 JPG/JPEG/PNG 上传、图片墙分页修复、归档清理任务化和关闭阶段日志崩溃风险修复统一纳入发布候选整理。
...
- 主机本地 JPG/JPEG/PNG 导入，支持选择文件夹批量导入，也支持从文件夹中手动选择单张或多张图片导入；大批量导入通过后台任务执行，任务中心显示进度、成功/跳过/失败数量、预计剩余时间和取消入口。
...
- 客户端模式：连接主机、读取活动、打开图片墙、下载图片、选择单张或多张 JPG/JPEG/PNG 上传到主机；上传后的图片处理进入后台任务并实时广播新图，可进入客户端修图任务页下载待修包和回传已修图。
...
- 任务中心与批量 ZIP 下载：提供内存任务队列、`task-updated` 实时推送、主机/客户端侧边栏任务中心、图片墙“下载所选 ZIP”；主机导入、客户端上传处理、发布导出、归档生成和归档清理工作区均可在任务中心查看进度和错误。
...
- `pnpm dist:win`：生成 Windows NSIS 安装包到 `release-pack/`，当前仅保留为后续评估入口；v0.14.0 人工测试发现安装器卡住，暂不推荐作为交付物。
...
打包产物输出到 `release-pack/`。`pnpm dist:portable` 当前使用 ZIP 便携包作为推荐交付物：解压 ZIP 后运行其中的 `Media Photo Workbench.exe`。v0.15.0-rc.0 推荐交付物为：

```text
MediaPhotoWorkbench-v0.15.0-rc.0-x64.zip
```
...
NSIS 安装包在 v0.14.0 人工验证中出现安装进度卡住、取消按钮无响应、需要强制结束安装器进程的问题，当前暂不提供或暂不推荐作为交付物。若未来重新提供安装包，会在 Release 和 README 中另行说明。
...
- 长任务：主机大批量导入、客户端多文件上传处理、批量 ZIP 下载、待修包生成、发布导出、归档流程能在任务中心看到状态；导入任务支持进度、ETA 和取消。

真实压力测试建议使用 50 / 300 / 500 张 JPG/JPEG/PNG 分批验证导入、图片墙滚动、批量下载、待修包、导出发布和归档；大批量导入建议额外用 4000 张混合素材验证后台任务进度、ETA、取消、重复导入 skipped 和关闭程序安全性。大量图片导入期间建议不要移动源文件或断开仓库所在磁盘。第一版暂不把手机端作为完整工作流适配目标。
...
- v0.14.0：故障排查 / 诊断信息入口、客户端连接失败提示优化、校园网 / 防火墙 / 热点排查提示、NSIS 初步验证；NSIS 暂不推荐，ZIP 便携包继续作为主交付物。
- v0.15.0-rc.0：导入性能与稳定性增强发布候选版，包含大批量导入任务化、PNG 支持、主机文件夹 / 指定图片导入、客户端 JPG/JPEG/PNG 上传、图片墙分页修复、归档清理任务化和关闭阶段日志崩溃风险修复。
- v0.15.0 或 v0.15.0-rc.1：根据回归测试结果决定正式发布或修复版。
