...
**当前开发阶段：v0.16.0（内部批次 16.1-16.5，暂不单独发布内部批次版本）**

项目已打通从主机建活动、本地导入、真实图片墙、单图下载、Socket.IO 实时同步、客户端上传协作、待修包生成、已修图回传、正式发布导出 ZIP、活动归档，到活动/图片回收站恢复和安全永久删除的核心闭环。当前已接入统一任务中心、批量 ZIP 下载、导入后台任务、归档活动只读打开、客户端修图任务页、自定义待修分包和桌面端非最大化窗口适配。v0.16.0 开发阶段在 v0.15.0 基础上继续补充图片墙实战修复、任务中心覆盖、拖拽导入 / 上传，以及数据库备份与数据库位置迁移。
...
- 客户端模式：连接主机、读取活动、打开图片墙、下载图片、选择或拖拽单张 / 多张 JPG/JPEG/PNG 上传到主机；客户端拖拽文件夹暂不支持。上传后的图片处理进入后台任务并实时广播新图，可进入客户端修图任务页下载待修包和回传已修图。
...
- v0.16.0-rc / v0.16.0：根据 16.x 回归测试结果决定是否先发布候选版或直接发布正式小版本。
